...
public class ForgotPasswordController {
    ...
    public ResponseEntity<ResponseObject> reset(@RequestParam("email") String email){
        ...
        jwtService.generateToken(service.findByEmail(email));
        //Kiểm tra email đã có mã otp,


        emailSenderService.sendEmail(email);
        passwordResetDAO.save(new PasswordReset(email,emailSenderService.code));
         return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject("Email had been send", "OK",jwtService.generateToken(service.findByEmail(email))));
    }
